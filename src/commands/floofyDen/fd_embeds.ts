...
module.exports = {
        ...
        .addSubcommand(subcommand =>
          subcommand
            ...
            .addBooleanOption(option =>
              option
                .setName('edit')
                .setDescription('Do you want to edit an existing message or post a new one')
                .setRequired(true)
            )
        )
        .addSubcommand(subcommand =>
  ...
  async execute(interaction: Discord.ChatInputCommandInteraction) {
    ...
    if (interaction.options.getSubcommand() === 'staff-guidelines') {
      await interaction.deferReply({ ephemeral: true });
      await interaction.guild?.roles.fetch(managementRoleId).then(async role => {
            ...
            guild.channels
              ...
              .then(async channel => {
                ...
                await channel.send({
                  ...
                  files: [new Discord.AttachmentBuilder('src/assets/staffGuidelines.png')],
                });
                await channel.send({
                  embeds: [
                    ...
                    Components.embed.floofyDen.staffGuidelines[8](
                      role?.members
                        .map((member: Discord.GuildMember) => member.user.toString())
                        .join(', ')
                    ),
                  ],
                  components: [
