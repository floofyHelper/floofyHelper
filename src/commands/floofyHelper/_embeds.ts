      ...
      guild.channels.fetch(`${interaction.options.get('channel')?.value}`).then(async channel => {
        ...
        channel
          .send({
            ...
            embeds: [
              Components.embed.commands.rules[1],
              await Components.embed.commands.rules[2](interaction),
              Components.embed.commands.rules[3],
              Components.embed.commands.rules[4],
              Components.embed.commands.rules[5],
            ],
          })
          .then(() =>
