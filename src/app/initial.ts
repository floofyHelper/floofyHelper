...
export const client3 = new MongoClient(process.env.mongoDbUri!) // Configure Database Uri
...
client.once('ready', () => {
	...
	client2.once('ready', () => {
		client3.db('admin').command({ ping: 1 })
		console.log(
			chalk.white(timestamp),
			chalk.underline.magentaBright('Startup'),
			chalk.greenBright(' Connected to database')
		)
		client3.once('connectionReady', () => {
			inquirer
				.prompt([
					{
						type: 'list',
							...
							.prompt([
								{
									type: 'list',
									message:
										'Which bot(s) do you want to boot?',
									name: 'development',
									choices: [
										{
