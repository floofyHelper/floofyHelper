...
console.log(chalk_1.default.white(initial_js_1.timestamp), chalk_1.default.underline.magentaBright('Startup'), ` ${initial_js_1.client2.user?.username} files found, starting bot...`);
...
function consoleLogError(err) {
    console.error(chalk_1.default.white(initial_js_1.timestamp), chalk_1.default.underline.blueBright(initial_js_1.client2.user?.username), ' ', err);
}
async function forumCheckForExistingThreadThenLog(forumChannelId, threadAuthorId, threadName, threadDescription, messageComponents) {
    try {
        ...
        channel.threads.fetch().then(async (collection) => {
            if (initial_js_1.client2.user?.id !== threadAuthorId)
                return;
...
async function sendToErrorLog(err, interaction) {
    try {
        ...
        await forumCheckForExistingThreadThenLog(process.env.errorLog, '989979801894912040', // CHANGE TO MAIN BOT ID BEFORE STAGING
        '🛑 Error Log', '## <:myBots:1001930208393314334> This channel is used to inform devs of errors with <@!953794936736727110>\n\n- Follow this channel to receive alerts', {
...
initial_js_1.client2.on('guildMemberAdd', async (member) => {
    try {
        ...
        buttons.components[0].setDisabled(false).setStyle(discord_js_1.default.ButtonStyle.Secondary);
        buttons.components[1].setDisabled(false).setStyle(discord_js_1.default.ButtonStyle.Secondary);
        buttons.components[2].setDisabled(false).setStyle(discord_js_1.default.ButtonStyle.Secondary);
        buttons.components[3].setDisabled(false).setStyle(discord_js_1.default.ButtonStyle.Secondary);
        // Adding guild & verification data to database
        await initial_js_1.client3
            .db('BaseInteraction')
            .collection('guild')
            .updateOne({ _id: member.guild.id }, {
            $set: {
    ...
    try {
        if (interaction.customId.startsWith('verification1 1')) {
            ...
            await interaction.client.guilds.cache
                .get(guildId)
                ?.members.kick(`${interaction.user.id}`, `User is under 13 | ${interaction.client.guilds.cache.get(guildId)?.name}`);
            const channel = initial_js_1.client2.channels.cache.get(config.testing.verificationChannel);
            if (channel?.isTextBased()) {
                await channel.send({
                    embeds: [components_js_1.embed.verificationReview(interaction, guildId)],
                });
            }
        }
        ...
        if (interaction.customId.startsWith('verification2 2')) {
            if (interaction.user.bot === false) {
                ...
                await message?.delete();
                await interaction.user.send({
                    embeds: [components_js_1.embed.verification(interaction.user, initial_js_1.client2.guilds.cache.get(id))],
                    components: [buttons],
                });
            }
        }
        ...
        if (interaction.customId === 'verificationHelp 2') {
            if (interaction.user.bot === false) {
                ...
                if (interaction.guild?.name === undefined) {
                    ...
                    await interaction.user.send({
                        embeds: [components_js_1.embed.verification(interaction.user, interaction.guild)],
...
initial_js_1.client2.on('interactionCreate', async (interaction) => {
    ...
    try {
        if (interaction.customId.startsWith('verification1')) {
            ...
            if (interaction.customId.split(',').at(2) === '2') {
                buttons.components[0].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Secondary);
                buttons.components[1].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Primary);
                buttons.components[2].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Secondary);
                buttons.components[3].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Secondary);
            ...
            if (interaction.customId.split(',').at(2) === '3') {
                buttons.components[0].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Secondary);
                buttons.components[1].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Secondary);
                buttons.components[2].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Primary);
                buttons.components[3].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Secondary);
                if (interaction.isFromMessage()) {
                    interaction.update({
                        components: [buttons],
            ...
            if (interaction.customId.split(',').at(2) === '4') {
                ...
                buttons.components[1].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Secondary);
                buttons.components[2].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Secondary);
                buttons.components[3].setDisabled(true).setStyle(discord_js_1.default.ButtonStyle.Primary);
                if (interaction.isFromMessage()) {
                    interaction.update({
                        components: [buttons],
                    });
            ...
            const id = interaction.customId.split(',');
            id.shift();
            const guildId = initial_js_1.client2.guilds.cache.get(interaction.customId.split(',').at(1));
            interaction.user
                .send({
                embeds: [components_js_1.embed.verification2(interaction, guildId?.iconURL())],
...
// -------------------------------------------------------------------------------
console.log(chalk_1.default.white(initial_js_1.timestamp), chalk_1.default.underline.magentaBright('Startup'), chalk_1.default.greenBright(` ${initial_js_1.client2.user?.tag} is logged in`));
